...
foreach ($projects as $p) {
    ...
    // video_type drives which player the frontend renders:
    //   'external' → <iframe> embed (YouTube / Vimeo)   — priority
    //   'local'    → <video>  tag   (self-hosted file)  — fallback
    //   ''         → auto-detect from available fields  (legacy rows)
    $storedVideoType = (string)($p["video_type"] ?? "");
    $videoUrl        = (string)($p["video_url"] ?? "");
    if ($storedVideoType === "") {
        $storedVideoType = ($videoUrl !== "") ? "external" : ($videoSrc !== "" ? "local" : "");
    }
