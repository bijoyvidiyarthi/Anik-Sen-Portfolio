...
(function () {
    ...
    function initProjectFilter() {
        ...
        function openVideoModal(p) {
            ...
            //   videoType === 'external'  → always embed via iframe (YouTube/Vimeo)
            //   videoType === 'local'     → use self-hosted <video> tag
            //   fallback (legacy rows)    → external URL if present, else local file
            const vType = (p.videoType || "").trim();
            const url   = (p.videoUrl  || "").trim();
            const file  = (p.videoFile || "").trim();
            let html = "";

            const useExternal = vType === "external" || (vType === "" && url !== "");
            const useLocal    = !useExternal && (vType === "local" || (vType === "" && file !== ""));

            if (useExternal && url) {
                // Embed YouTube/Vimeo or any parseable URL as an iframe.
                html = embedHtml(url);
            } else if (useLocal && file) {
                // Self-hosted file: native <video> with poster and controls.
                ...
                // Ultimate fallback: any URL we have.
                html = embedHtml(url);
            } else if (file) {
                const mime   = (p.videoMime || "").trim();
                const poster = (p.poster    || "").trim();
