...
try {
    ...
    // Compute the true unique visitor count across the full period — DISTINCT
    // ip_hash so the same person visiting on multiple days counts only once.
    $distinctTotal = match ($range) {
        "weekly"  => Visitor::distinctInRange(
            $today->modify("-6 days")->format("Y-m-d"),
            $today->format("Y-m-d")
        ),
        "monthly" => Visitor::distinctInRange(
            $today->modify("-29 days")->format("Y-m-d"),
            $today->format("Y-m-d")
        ),
        "yearly"  => Visitor::distinctInRange(
            (new \DateTimeImmutable("first day of this month"))->modify("-11 months")->format("Y-m-d"),
            $today->format("Y-m-d")
        ),
    };
