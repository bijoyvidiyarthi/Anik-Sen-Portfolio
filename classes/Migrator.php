...
class Migrator
{
    ...
    /**
     * Parse a raw YouTube or Vimeo share URL into a clean embed URL.
     * Returns the input unchanged if no pattern matches (passes custom embeds through).
     *
     * Supported inputs:
     ...
     *   https://player.vimeo.com/video/VIDEO_ID (already embed — returned as-is)
     */
    public static function toEmbedUrl(string $url): string
    {
        ...
        // YouTube patterns
        $yt = '/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([\w\-]{6,})/i';
        ...
        return $url; // custom or already-embed URL — pass through unchanged
    }

    /** Derive video_type from submitted fields.
     *  'external' → external URL will be shown in an <iframe>
     *  'local'    → self-hosted file will be shown in a <video> tag
     *  ''         → no video configured
     */
    public static function resolveVideoType(string $videoUrl, ?string $videoFile, string $existingType = ''): string
    {
        if ($videoUrl !== '') {
            return 'external';
        }
        ...
        // Both URL and file are empty — no video at all; never carry a stale type forward.
        return '';
    }
    ...
    private static function runIncrementalMigrations(): void
    {
        ...
        // --- v14: explicit video_type column on projects ---
        // Distinguishes 'external' (YouTube/Vimeo iframe) from 'local' (self-hosted file).
        // This is derived automatically on save and stored so the frontend never has to guess.
        if (!self::columnExists('projects', 'video_type')) {
            $pdo->exec("ALTER TABLE projects ADD COLUMN video_type VARCHAR(20) NOT NULL DEFAULT ''");
            // Backfill: derive type from existing data for all current rows.
            $rows = $pdo->query("SELECT id, video_url, video_file FROM projects")->fetchAll();
            $upd  = $pdo->prepare("UPDATE projects SET video_type=:vt WHERE id=:id");
            foreach ($rows as $r) {
                $vt = self::resolveVideoType(
                    (string)($r['video_url'] ?? ''),
                    ($r['video_file'] !== '' && $r['video_file'] !== null) ? $r['video_file'] : null,
                    ''
                );
                if ($vt !== '') {
                    $upd->execute([':vt' => $vt, ':id' => (int)$r['id']]);
                }
            }
        }
        ...
        // --- v18: revert visitor_log unique index to reliable IP-based deduplication ---
        // v14 switched the unique key to (visitor_token, day_key), where visitor_token
        // comes from a browser cookie. On HTTP hosts (no HTTPS) or when cookies are not
        // preserved between requests, a new random token is generated on every page load,
        // which bypasses the uniqueness check and inflates the visitor count on every
        // refresh by the same person. Reverting the index to (ip_hash, day_key) makes
        // deduplication reliable regardless of cookie behaviour.
        $marker18 = $pdo->query("SELECT value FROM settings WHERE `key`='visitor_ip_dedup_fix_v18' LIMIT 1")->fetchColumn();
        if (!$marker18) {
            // 1. Remove duplicate rows — keep only the first (lowest id) per ip+day.
            //    Wrapped in a nested subquery so it works on both MySQL and SQLite.
            try {
                $pdo->exec(
                    "DELETE FROM visitor_log WHERE id NOT IN (
                        SELECT id FROM (
                            SELECT MIN(id) AS id FROM visitor_log GROUP BY ip_hash, day_key
                        ) AS _keep
                    )"
                );
            } catch (\Throwable $e) { /* table may be empty — ignore */ }

            // 2. Drop the cookie-based unique index introduced in v14.
            try {
                if ($isMysql) {
                    $pdo->exec("ALTER TABLE visitor_log DROP INDEX ux_visitor_token_day");
                } else {
                    $pdo->exec("DROP INDEX IF EXISTS ux_visitor_token_day");
                }
            } catch (\Throwable $e) { /* index may not exist on fresh installs */ }

            // 3. (Re-)create the IP+day unique index.
            try {
                $pdo->exec("CREATE UNIQUE INDEX ux_visitor_ip_day ON visitor_log (ip_hash, day_key)");
            } catch (\Throwable $e) { /* already exists on fresh installs from v9 */ }

            $pdo->prepare("INSERT INTO settings (`key`, value) VALUES ('visitor_ip_dedup_fix_v18', '1')")->execute();
        }

        // --- v19: one row per unique visitor, ever ---
        // Previous strategies (per-day dedup by IP or cookie) still inflated
        // the "Unique Visitors" total when the same browser visited on multiple
        // days. v19 switches to a single-row-per-visitor model:
        //   • UNIQUE index on visitor_token alone (not visitor_token+day_key)
        //   • Cookie TTL extended to 1 year so the token survives across sessions
        //   • day_key now stores the date of the visitor's VERY FIRST arrival
        //   • INSERT OR IGNORE on a known token is a complete no-op forever
        // The daily/monthly/yearly charts now show "new unique arrivals" per
        // period (visitors whose first visit fell in that window).
        $marker19 = $pdo->query("SELECT value FROM settings WHERE `key`='visitor_true_unique_v19' LIMIT 1")->fetchColumn();
        if (!$marker19) {
            // 1. Keep only the oldest row per visitor_token (their first visit).
            try {
                $pdo->exec(
                    "DELETE FROM visitor_log WHERE id NOT IN (
                        SELECT id FROM (
                            SELECT MIN(id) AS id FROM visitor_log GROUP BY visitor_token
                        ) AS _keep
                    )"
                );
            } catch (\Throwable $e) {}

            // 2. Drop the ip-based unique index added by v18.
            try {
                if ($isMysql) {
                    $pdo->exec("ALTER TABLE visitor_log DROP INDEX ux_visitor_ip_day");
                } else {
            ...
            // 3. Create the new per-token unique index.
            try {
                $pdo->exec("CREATE UNIQUE INDEX ux_visitor_token ON visitor_log (visitor_token)");
            } catch (\Throwable $e) {}
